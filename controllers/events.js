...
const updateStreak = async (actorId) => {
  ...
  sub_streak = isStreak(first.created_at, second.created_at)
    ? first.actor.sub_streak + 1 : 0;
  main_streak = sub_streak > first.actor.main_streak
    ? sub_streak : first.actor.main_streak;

  await actors.update({ main_streak, sub_streak }, { where: { id: actorId } });
};
...
const formatDate = (results) => results.map((result) => {
  ...
  const created_at = newTime.toISOString()
...
export const addEvent = async (req, res, next) => {
  try {
    ...
    await events.create({
      ...event, actorId: actor.id, repoId: repo.id,
    });
